...
import type { FhirPatient, FhirBundle } from '@open-hims/types';
...
export const handlers = [
  ...
  http.get('/fhir/Patient', ({ request }) => {
    ...
    if (family) {
      filteredPatients = mockPatients.filter(p => 
        p.name?.[0]?.family?.toLowerCase().includes(family.toLowerCase())
      );
    }
  ...
  http.post('/fhir/Patient', async ({ request }) => {
    const patient = await request.json() as FhirPatient;
    const newPatient = {
      ...patient,
      id: `patient-${Date.now()}`,
      meta: {
  ...
  http.put('/fhir/Patient/:id', async ({ params, request }) => {
    ...
    const updatedPatient = await request.json() as FhirPatient;
